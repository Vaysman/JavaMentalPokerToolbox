...
public class TMCGCard implements Card {
    ...
    private class TMCGCardSecret implements CardSecret {
        private boolean[][] b;
        private int bits;
        private int players;
    ...
    public TMCGCard mask(PublicKeyRing ring, boolean timingAttackProtection) {
        assert ring.size() == players;
...
//        assert cs.getBitsCount() == bits;
        ...
        for (int k = 0; k < players; k++) {
            for (int w = 0; w < bits; w++) {
                ...
                masked.setZ(k, w, Utils.maskValue(z[k][w], secret.getRandom(k, w), key.getY(), key.getModulus(), cs.getBit(k, w), timingAttackProtection));
            }
        }
        return masked;
    }
